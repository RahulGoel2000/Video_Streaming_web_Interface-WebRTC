function detectMob() {
  ...
  const sharescree=document.createElement("video");
  myVideo.muted = true;
  var myVideoStream;
  var myVideoTrack;
  const peers = {};
 ...
 // External toggles
  ...
  videoToggleBtn.addEventListener("click", (e) => {
    ...
    if (enabled) {
      ...
      currentElement.classList.add("afterbuttoncolor");
      // currentElement.setAttribute("background", "green");
    } else {
      ...
      currentElement.classList.remove("afterbuttoncolor");
      // currentElement.setAttribute("background", "red");
    }
  });
  ...
  micToggleButton.addEventListener("click", (e) => {
    ...
    if (enabled) {
      ...
      // currentElement.setAttribute("background-color", "Red");
      currentElement.classList.remove("afterbuttoncolor");
    }
  });
  ...
  socket.emit("trying", ROOM_ID,name,USER_ID);
...
const recordingBtn = document.getElementById("recording-toggle");
 ...
 {shareScreenBtn.remove();
  ...
  const addMessage = (sender, userName, message,time) => {
  ...
  chatForm.addEventListener("submit", (e) => {
    ...
    addMessage("me", name, chatInput.value, new Date());
    scrollDown(".chat-box");
    chatInput.value = "";
  });
  
  socket.on("client-podcast", (data, userName,roomsId) => {
    // console.log(ROOM_ID);
    // console.log(roomsId);
    if(roomsId==ROOM_ID)
    {
    addMessage("user", userName, data,new Date());
    scrollDown(".chat-box");}
  });
  
  socket.on('output-messages', (data, userName,usresId) => {
    ...
    if (data.length) {
        data.forEach(message => {
          ...
          if(message.UserID==USER_ID)
            addMessage("me",message.sender,message.message,new Date(message.createdAt));
          else
            addMessage("user",message.sender,message.message,new Date(message.createdAt));
        });
    }
});


// mail-link to suggested ID invite link

//   const mailForm = document.querySelector(".share-mail-wrapper");
//   mailForm.addEventListener("send", (e) => {
...
//   from: 'rahulsapnagoel@gmail.com',
//   to: 'rahulgoelb@gmail.com',
//   subject: 'Sending Email using Node.js',
//   text: `Hi Smartherd, thank you for your nice Node.js tutorials.
//           I will donate for this course. Please send me payment options.`
